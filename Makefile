...
# below, $(S)* is a bit of a hack; the original is: $(OBJ_DIR)$S$(SOURCE_NAME).$O \    this may not work perfectly, but wtvr. (the changed version: $(OBJ_DIR)$(S)* \ )
...
.PHONY: compile # Compile obj file.
compile: $(OBJ_DIR)/$(SOURCE_NAME).$O
